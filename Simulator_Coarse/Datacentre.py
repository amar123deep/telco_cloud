...
class Datacentre(Resource):
	...
	RESOURCE_TYPES = {
		"L":{ 
				"CPU":		{'CAPACITY':700.0, 'EXECOST': LinearCostFunc(1,0.0),'OVERLOADCOST': BarrierFunc(1,0),'MU':1},
		...
		"M":{ 
				"CPU":		{'CAPACITY':100.0, 'EXECOST': LinearCostFunc(2,0.0),'OVERLOADCOST': BarrierFunc(1,0),'MU':1},
				"NET_UP":	{'CAPACITY':100.0, 'EXECOST': NoCostFunc(),'OVERLOADCOST':BarrierFunc(1,0),'MU':1},
				"NET_DOWN":	{'CAPACITY':100.0, 'EXECOST': NoCostFunc(),'OVERLOADCOST': BarrierFunc(),'MU':1}
			},
		"S":{ 
				"CPU":		{'CAPACITY':50.0, 'EXECOST': LinearCostFunc(4,0.0),'OVERLOADCOST': BarrierFunc(1,0),'MU':1},
				"NET_UP":	{'CAPACITY':50.0, 'EXECOST': NoCostFunc(),'OVERLOADCOST': BarrierFunc(),'MU':1},
				"NET_DOWN":	{'CAPACITY':50.0, 'EXECOST': NoCostFunc(),'OVERLOADCOST': BarrierFunc(),'MU':1}
			}
		}
